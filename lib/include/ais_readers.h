...
//	ACTION_REJECTED_OTHER, // ?
	...
	ACTION_CARD_DISCARDED_NO_CREDIT,
	ACTION_CARD_DISCARDED_CREDIT_TIME,
	ACTION_CARD_DISCARDED_CREDIT_OTHER,
...
/**
 * Clear the list with custom communication interfaces,
 * added with AIS_IF_Add()
 */
DL_API void AIS_IF_ClearAll(void);

/**
 * This function adds custom IP address in the list for interface checking.
 * This list appends to the list of discovered interfaces via broadcast
 * before try to open AIS Reader device
 *
 * AIS_IF_ClearAll() erase all inserted IP addresses
 *
 * @param address specific information for I/F, like IP address "192.168.1.155"
 * @param port
 * @param protocol_type 'T' for TCP/IP or 'U' for UDP
 * @param com_baudrate in bps
 * @return DL_STATUS
 */
DL_API
DL_STATUS AIS_IF_Add(c_string address, int port,
		char protocol_type, uint32_t com_baudrate);

/**
 * Function returns the list of custom communication interfaces in format:
 *   for IP address INET: "IF{192.168.111.222:58881,T,250000}\n"
 *
 * @return pointer to already allocated space on heap (in the library)
 *           with C-string information
 */
DL_API
c_string AIS_IF_GetAll(void);

////////////////////////////////////////////////////////////////////
/// Device types, speed up opening
////////////////////////////////////////////////////////////////////
...
/**
 *
 * @param pDevice_HND assigned Handle
 * @param pDevice_Serial device serial number
 * @param pDevice_Type : device identification in AIS database, look at enum E_KNOWN_DEVICE_TYPES / device_e from the file : ais_readers_list.h
 * @param pDevice_ID device identification number (master)
 * @param pDevice_FW_VER version of firmware
 * @param pDevice_CommSpeed communication speed
 * @param pDevice_FTDI_Serial FTDI COM port identification
 * @param pDevice_isOpened is Device opened
 * @param pDevice_Status actual device status
 * @param pSystem_Status actual system status
 * @return
 */
DL_API
 ...
 *    and wait for command_finish become not zero (true).
...
DL_STATUS AIS_Blacklist_Write_byType(HND_AIS device, c_string password,
		c_string str_csv_blacklist, int32_t bl_enum);
 ...
 */
DL_API
DL_STATUS AIS_DeviceTypeInfo(IN device_e dev_type, OUT c_string *name,
